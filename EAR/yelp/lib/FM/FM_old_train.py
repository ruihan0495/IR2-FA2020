...
with open('../../data/FM-train-data/inverted_dict-new.pickle', 'rb') as f:
...
def train(model, bs, max_epoch, optimizer1, optimizer2, optimizer3, reg, qonly, observe, command, filename, uf, use_useremb):
    ...
    for epoch in range(max_epoch):
        ...
        if epoch % observe == 0 and epoch > -1:
            ...
            evaluate_7(model, epoch, filename, 0)
        ...
        for iter_ in range(max_iter):
            ...
            result_pos, feature_bias_matrix_pos, nonzero_matrix_pos = model(pos_list, pos_list2, preference_list_1)  # (bs, 1), (bs, 2, 1), (bs, 2, emb_size)
...
def main():
    ...
    if A.pretrain == 2:
        # means we load pretrain model
        model = FactorizationMachine(emb_size=A.hs, user_length=user_length, item_length=item_length,
                                     feature_length=FEATURE_COUNT, qonly=A.qonly, command=A.command, hs=A.hs, ip=A.ip, dr=A.dr, old_new=A.oldnew)
        fp = '../../data/FM-model-merge/v32-test-FM-lr-0.01-flr-0.0001-reg-0.002-decay-0.0-qonly-1-bs-64-command-8-hs-64-ip-0.01-dr-0.5-optim-{}-oldnew-new-pretrain-0-uf-0-rd-0-freeze-0-seed-3704-useremb-1epoch-49.pt'.format(A.optim)
        print(fp)
    ...
    file_name = 'v32-test-FM-lr-{}-flr-{}-reg-{}-decay-{}-qonly-{}-bs-{}-command-{}-hs-{}-ip-{}-dr-{}-optim-{}-oldnew-{}-pretrain-{}-uf-{}-rd-{}-freeze-{}-seed-{}-useremb-{}'.format(A.lr, A.flr, A.reg, A.decay, A.qonly,
