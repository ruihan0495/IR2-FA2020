...
def rank_by_batch(pickle_file, iter_, bs, pickle_file_length, model, rd):
    '''
...
def evaluate_7(model, epoch, filename, rd):
    model.eval()
    tt = time.time()
    pickle_file_path = '../../data/FM-sample-data-new/v1-speed-valid-{}.pickle'.format(1)
    ...
    for iter_ in range(max_iter):
        ...
        result += rank_by_batch(pickle_file, iter_, bs, pickle_file_length, model, rd)
