...
class agent():
    def __init__(self, FM_model, user_id, busi_id, do_random, write_fp, strategy, TopKTaxo, numpy_list, PN_model, log_prob_list, action_tracker, candidate_length_tracker, mini, optimizer1_fm, optimizer2_fm, alwaysupdate, do_mask, sample_dict, choose_pool):
    ...
    def mini_update_FM(self):
        ...
        neg_items = self.rejected_item_list_[-10:]  # use recently rejected 10 items.
    ...
    def vectorize(self):
        ...
        # For the ablation study on the state vector
        if cfg.mask == 1:
            list3 = [0] * len(list3)
        if cfg.mask == 2:
            list4 = [0] * len(list4)
    ...
    def vectorize_crm(self):
        # We strictly follow the description in CRM (https://arxiv.org/abs/1806.03277)
        # They use the state tracker as state vector. Our implementation is the upper-bound for their state tracker, since the natural language understanding will also introduce error.
            ...
            if value is not None and value[0] is not None:
                ...
                if facet == 'stars':
                    if int(value[0] + cfg.city_count) not in self.known_feature:
                        self.known_feature.append(int(value[0] + cfg.city_count - 1))
                    else:
                        fresh = False
                ...
                if cfg.play_by != 'AOO' and cfg.play_by != 'AOO_valid':
                    ...
                    self.recent_candidate_list_ranked, self.previous_dict = rank_items(self.known_feature, self.user_id, self.busi_id, self.skip_big_feature, self.FM_model, self.recent_candidate_list, self.write_fp, 0, self.rejected_item_list_, self.previous_dict)
        # end if
        ...
        if input_message.message_type == cfg.REJECT_REC:
            ...
            if self.mini == 1:
                if self.alwaysupdate == 1:
                    ...
                    self.recent_candidate_list = list(set(self.recent_candidate_list) - set([self.busi_id])) + [self.busi_id]
                    self.recent_candidate_list_ranked, self.previous_dict = rank_items(self.known_feature, self.user_id, self.busi_id, self.skip_big_feature, self.FM_model, self.recent_candidate_list, self.write_fp, 1, self.rejected_item_list_, self.previous_dict)
