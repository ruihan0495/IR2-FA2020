...
def main():
    ...
    for epi_count in range(A.startFrom, A.endAt):
        ...
        for name, param in current_FM_model.named_parameters():
            if i == 0:
                param1.append(param)
                # param1[0]: torch.Size([1])
            else:
                param2.append(param)
                # param2[0]: torch.Size([97987, 65])
                # param2[1]: torch.Size([591, 65])
            i += 1
        ...
        if A.purpose != 'fmdata':
            ...
            if A.upoptim == 'Ada':
                optimizer2_fm = torch.optim.Adagrad(param2, lr=0.01, weight_decay=A.decay)
            if A.upoptim == 'SGD':
        ...
        for c in choose_pool:
            ...
            if A.purpose != 'pretrain':
                log_prob_list, rewards = run_one_episode(current_FM_model, user_id, item_id, A.mt, False, write_fp,
                                                         ...
                                                         optimizer1_fm, optimizer2_fm, A.alwaysupdate, start_facet, A.mask, sample_dict, choose_pool_original)
            else:
